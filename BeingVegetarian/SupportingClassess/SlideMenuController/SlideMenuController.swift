...
extension UIViewController {
    ...
    public func addRightBarButtonWithImage() {
        ...
        btn1.setImage(UIImage(named: "Notifiction"), for: .normal)
        btn1.frame = CGRect(x: 0, y: 0, width: 30, height: 30)
        btn1.addTarget(self, action: #selector(toggleRight), for: .touchUpInside)
        let item1 = UIBarButtonItem(customView: btn1)
        let btn2 = UIButton(type: .custom)
        btn2.setTitle("QueenStwon", for: .normal)
        btn2.titleLabel?.textColor = UIColor.white
        btn2.titleLabel?.font = UIFont.systemFont(ofSize: 14.0, weight: .bold)
        btn2.frame = CGRect(x: 0, y: 0, width: 30, height: 30)
        btn2.addTarget(self, action: #selector(toggleRight), for: .touchUpInside)
